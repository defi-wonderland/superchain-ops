...
contract RegressionTest is Test {
    ...
    /// @notice Expected call data and data to sign generated by manually running the RevShareUpgradeAndSetup template at block 22447773 on mainnet.
    /// Simulate from task directory (test/tasks/example/eth/016-revshare-upgrade-and-setup) with:
    /// SIMULATE_WITHOUT_LEDGER=1 just --dotenv-path $(pwd)/.env --justfile ../../../../../src/nested.just simulate <foundation|council>
    function testRegressionCallDataMatches_RevShareUpgradeAndSetupp() public {
        string memory taskConfigFilePath = "test/tasks/example/eth/016-revshare-upgrade-and-setup/config.toml";
        string memory expectedCallData =
            "";
        MultisigTask multisigTask = new RevShareUpgradeAndSetup();
        address rootSafe = address(0x6d5B183F538ABB8572F5cD17109c617b994D5833);
        address foundationChildMultisig = 0x847B5c174615B1B7fDF770882256e2D3E95b9D92;
        address[] memory allSafes = MultisigTaskTestHelper.getAllSafes(rootSafe, foundationChildMultisig);

        (Action[] memory actions, uint256[] memory allOriginalNonces) =
            _setupAndSimulate(taskConfigFilePath, 23819861, "mainnet", multisigTask, allSafes);

        _assertCallDataMatches(multisigTask, actions, allSafes, allOriginalNonces, expectedCallData);

        // Data to sign generated by manually running the TransferL2PAOFromL1 template at block 23819861 on mainnet.
        string[] memory expectedDataToSign = new string[](2);
        
        // Foundation
        expectedDataToSign[0] =
            "0x1901a4a9c312badf3fcaa05eafe5dc9bee8bd9316c78ee8b0bebe3115bb21b7326727ba5c4740e7702a0501739e625d1c49867a16c60ede5ad66ae760c33134a8c6f";
        // Security council
        expectedDataToSign[1] =
            "0x1901df53d510b56e539b90b369ef08fce3631020fbf921e3136ea5f8747c20bce9670caa76cd73550f0b37888afeb21bf1c7a640bc966fbf3d20c6dbceb77256572e";
        _assertDataToSignNestedMultisig(multisigTask, actions, expectedDataToSign, MULTICALL3_ADDRESS, rootSafe);
    }
    ...
    /// SIMULATE_WITHOUT_LEDGER=1 just --dotenv-path $(pwd)/.env --justfile ../../../../../src/improvements/single.just simulate <foundation|council>
    function testRegressionCallDataMatches_RevShareContractsUpgraderSepolia() public {
